...
> Codex migration note: before executing this workflow, apply the environment and tool mapping in `skills/webnovel-codex/SKILL.md`. Resolve `CLAUDE_PLUGIN_ROOT` to the installed Codex plugin root and treat `/webnovel-query` as a natural-language Codex intent.
